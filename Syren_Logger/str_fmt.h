/** Header file for the StringFormat class, which is used to format log messages.
 * 
 * @details
 * This file contains the definition of the StringFormat class, which is responsible for formatting
 * log messages with date-time information, logger name, log level, and message content. It supports
 * C# style positional string formatting and allows customization of the output format.
 */
...
class StringFormat {
	...
	/**
	 * @struct DateTime
	 * @brief Private structure to handle getting date-time data depending on the set time zone.
	 *
	 * This structure is responsible for generating and storing date-time information based on the
	 * specified time zone. It provides methods to retrieve individual components of the date and time.
	 */
private:
...
private:
	...
	std::string string_convert(float value) {
		...
		if (set_precision) {
			int length = 0;
			if (decimal_places != 0) {
				length = output.find('.') + decimal_places + 1;
			}
			else
				length = output.find('.');
...
/*
	...
	std::string string_convert(T value) {
		...
		static_assert(5 < 1, "Error: positional formating failed - formatting is defined only for std::strings and c++ primitive types (int, float, ... etc.)");
